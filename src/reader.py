...
def read_slp(file, loud=False):
    ...
    with h5py.File(file, "r") as f:
        ...
        if loud:
            ...
            print(
                track_names.shape
            )  # 1. SPALTE in tracks, am Ende nur 3 - gibt dir den getrackten Fisch
            print(track_names[0:9])
            print("\nTRACK OCCUPANCY")
            print(
                track_occupancy.shape
            )  # Boolean in welchem Frame ein Track aktiv ist
            print(track_occupancy[0, 0])
            print(track_occupancy[11000, 0])
            print("\nTRACKS")
            print(
                tracks.shape
            )  # der heiße scheiß, zweite SPALTE SIND X UND Y, ICH WEIß LEIDER NICHT WIE RUM (aber ist ja eigentlich auch erstmal egal), 4. Spalte obviously die Frames
...
def extract_rows(
    file, nodes_to_extract, fish_to_extract=[0, 1, 2], verbose=False
):
    """
    ...
    if verbose:
        ...
        for x in range(n_fishes):
            print(
                "{} - {} frames".format(
                    x, len(np.where(track_occupancy[:, x] == 1)[0])
                )
            )
        ...
        while curr_row < n_row:
            if not np.isnan(data[curr_row, col]):
                ...
                if curr_row >= n_row:
                    ...
                    step_count = 1
                    while last_not_nan_row < curr_row:
                        data[last_not_nan_row, col] = (
                            last_real_value - step * step_count
                        )
...
def correct_wrongly_interpolated_outliers(
        ...
        for outlier in i_out:
            ...
            if abs(dis) > max_tolerated_movement:
                if outlier + 1 in i_out and outlier + 2 in i_out:
                    ...
                    correct_outlier(
                        data, col1, col2, outlier, max_tolerated_movement
                    )
    ...
    for col in range(int(n_cols / 2)):
        ...
        for outlier in i_out:
            ...
            if abs(curr_dis) > max_tolerated_movement:
                ...
                while (
                    avg_dis(
                        data, col1, col2, i_start, i_end, (i_end - i_start + 1)
                    )
...
def extract_coordinates(
    file: str,
    nodes_to_extract: List,
    fish_to_extract: List[int] = [0, 1, 2],
    interpolate_nans: bool = True,
    interpolate_outlier: bool = True,
    i_o_rec: bool = False,
    verbose: bool = False,
) -> np.ndarray:
    """
